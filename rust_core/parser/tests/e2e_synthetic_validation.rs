...
fn test_e2e_synthetic_validation() {
    ...
    let artifacts_dir_raw = Path::new("../../test_artifacts/e2e_synthetic_validation"); fs::create_dir_all(&artifacts_dir_raw).expect("Failed to create dir"); let artifacts_dir_pathbuf = artifacts_dir_raw.canonicalize().unwrap(); let artifacts_dir = artifacts_dir_pathbuf.as_path();
