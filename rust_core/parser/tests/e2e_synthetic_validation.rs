...
fn test_e2e_synthetic_validation() {
    ...
    }
    {
        ...
        agent_harness::db::init_agent_db(&agent_conn).unwrap();
    ...
    for (i, input) in inputs.iter().enumerate() {
        ...
        state.databases.agent_db.execute(
            ...
            params![sec_id.clone(), doc_id.clone(), "Intro", 1, 0],
        ).unwrap();
        
        state.databases.agent_db.execute(
            "INSERT INTO blocks (id, section_id, document_id, block_type, content, sort_order) VALUES (?, ?, ?, ?, ?, ?)",
            params![uuid::Uuid::new_v4().to_string(), sec_id.clone(), doc_id.clone(), "paragraph", "Content", 0],
        ).unwrap();
